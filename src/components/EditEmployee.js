...
function EditEmployee({employees, updateEmployee, id}) {
  const [show, setShow] = useState(false);
  const [name, setName] = useState(employees.name)
  const [role, setRole] = useState(employees.role)
    ...
    <>
      ...
      >
        ...
        <form 
            onSubmit={(e) =>{
                handleClose()
                //PREVENT THE SCREEN FROM LOADING WHEN SUBMIT
                e.preventDefault()
                console.log(id, name, role)

                //THIS FUNCTION WAS GRAB INTO THE EMPLOYEE COMPONENTS TO ACCESS THE PARAMETERS WHICH CAME FROM APP JS
